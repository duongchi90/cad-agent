...
def _publication_file_prepared(value: object) -> dict[str, object]:
    ...
    if value["state"] not in _PUBLICATION_FILE_STATES:
        raise _publication_file_error("PUBLICATION_STAGE_INVALID")
    path_fields = ("target_path", "candidate_path", "backup_path", "stage_path")
    original_paths = {
        name: _publication_file_original_path(value[name]) for name in path_fields
    }
    paths = {
        name: _publication_file_resolve_checked(original_paths[name]) for name in path_fields
    }
