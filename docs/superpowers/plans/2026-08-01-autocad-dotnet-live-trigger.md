...
|---|---|---|---|---|---|---|---|---|
| T09 | Trigger `CADAGENT_DISPATCH` from Windows and test the .NET path on a disposable drawing | Integrated Option A head `e57f48b` | `codex/autocad-dotnet-t09-live-trigger` | `mcp_integration_lib/dotnet_ipc.py`, `mcp_integration_lib/tests/test_dotnet_ipc.py`, `mcp_integration_lib/tests/test_dotnet_ipc_live.py` | All C# files, `mcp_client.py`, `reviewer2.py`, `repair2.py`, `scripts/verify.ps1`, `docs/STATUS.md`, contracts, production drawings | None; sequential after the integration head | Focused pytest + Ruff; opt-in live test must be PASS, SKIP, or NOT RUN | Offline trigger tests and existing contract tests pass, live test is safe/disposable and explicitly marked, write-set/review clean. |

## Wave schedule

- **WAVE 5A:** PO writes this spec, plan, and task card; Coder T09 starts from
  exact base `e57f48b` in its own branch/worktree.
